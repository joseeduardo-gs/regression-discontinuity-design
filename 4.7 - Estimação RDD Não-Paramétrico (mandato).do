*****************************************************************************************
*                ANÁLISE EMPÍRICA - RDD NÃO-PARAMÉTRICO (MANDATO)
****************************************************************************************
...
rdrobust log_despesa_geral_pc margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_1

rdrobust log_despesa_geral_pib margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_2

rdrobust log_total_receitas_pc margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_3

rdrobust log_total_receitas_pc margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_4

rdrobust log_receita_tributaria_pc margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_5

rdrobust log_receita_tributaria_pib margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_6

rdrobust log_servidores_comissionados_mil margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_7

rdrobust educacao_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_8

rdrobust saude_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_9

rdrobust assistencia_social_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_10

rdrobust urbanismo_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_11

rdrobust transporte_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_12

rdrobust desporto_e_lazer_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_13

rdrobust seguranca_publica_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
estimates store rd_conditional_14

rdrobust gestao_ambiental_prop margem_vitoria_esquerda, c(0) vce(nncluster cod_municipio_ibge) all covs(pop fracao_pop_masculina fracao_pop_urbana pib_pc)
